...
export default function SignupPage() {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (step === 1) {
      if (!canContinue) {
        setError("لطفاً نام، نام خانوادگی و شماره موبایل معتبر وارد کنید.");
        return;
      }
      setStep(2);
      return;
    ...
    if (!form.agree) {
      setError("برای ثبت‌نام باید با قوانین و حریم خصوصی موافقت کنید.");
      return;
    }
    ...
    try {
      ...
      const hasDuplicate = users.some(
        (user: { email: string; mobile: string }) =>
          user.email.toLowerCase() === form.email.trim().toLowerCase() || user.mobile === form.mobile.trim()
      );

      if (hasDuplicate) {
        setError("برای این ایمیل یا شماره موبایل قبلاً حساب ساخته شده است.");
      ...
      setSuccess("حساب شما با موفقیت ساخته شد. در حال انتقال به صفحه ورود...");

      window.setTimeout(() => {
        router.push(`/auth/login?registered=1&email=${encodeURIComponent(form.email.trim().toLowerCase())}`);
      }, 900);
    } catch {
                ...
                {step === 1 ? (
                  <>
                    ...
                    <button
                      ...
                      className="w-full rounded-2xl bg-[#f3ddbb] py-3.5 text-base font-bold text-[#3c220c] shadow-[0_10px_24px_rgba(15,10,5,0.25)] transition-colors hover:bg-white disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      ادامه
                    </button>
                  </>
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="h-12 w-full rounded-2xl border border-white/15 bg-white/14 px-4 pr-11 pl-11 text-sm text-white placeholder:text-[#f0d3aa]/60 focus:border-[#f1d5ad]/60 focus:outline-none focus:ring-4 focus:ring-[#d4a96a]/15"
                        />
                        <svg className="absolute right-3.5 top-3.5 text-[#f0d3aa]" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    ...
                    <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-white/15 bg-white/10 p-3.5">
                      <input
                        type="checkbox"
                        checked={form.agree}
                        onChange={(e) => setForm({ ...form, agree: e.target.checked })}
                        className="mt-0.5 h-4 w-4 shrink-0 rounded accent-[#6d4014]"
                      />
                      <span className="text-sm leading-6 text-[#f7ead3]">
                        با{" "}
                        <Link href="/terms" className="font-medium text-white hover:text-[#f6dfbc] hover:underline">
                          قوانین و مقررات
                        </Link>{" "}
                        و{" "}
                        <Link href="/privacy" className="font-medium text-white hover:text-[#f6dfbc] hover:underline">
                          حریم خصوصی
                        </Link>{" "}
                        پریسما شاپ موافقم
                      </span>
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        type="submit"
                        disabled={!form.agree || form.password.length < 8 || isSubmitting}
