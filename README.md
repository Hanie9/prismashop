...
- صفحه جزئیات هر محصول با کنترل موجودی
- سبد خرید با مدیریت تعداد، کد تخفیف و محدودیت موجودی
- صفحه پرداخت و ثبت سفارش (سفارش در پنل ادمین ذخیره می‌شود)
- **پنل ادمین کامل** در مسیر `/admin` شامل:
  - داشبورد
  - مدیریت محصولات و تصاویر
  - دسته‌بندی‌ها
  - سفارش‌ها و وضعیت ارسال
  - مشتریان
  - کدهای تخفیف
  - موجودی انبار و هشدار کم‌موجودی
...
- مخفی شدن `Navbar` و `Footer` در صفحات `auth` و `admin`

### ورود به پنل ادمین

- آدرس: `/admin/login`
- ایمیل: `admin@prismashop.ir`
- رمز: `admin123`

داده فروشگاه (محصولات، سفارش‌ها، تخفیف‌ها و موجودی) در `localStorage` مرورگر ذخیره می‌شود.
